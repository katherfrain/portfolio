...
import SpawnParticle from './SpawnParticles.js';
...
function spawnParticle(html, style, tile){
  ...
  activeParticles.push({html:html, style: newStyle, time: new Date().getTime()+3000});
}
  ...
  tryMoveToSUB(tile) {
    ...
    switch(tile.type){
      ...
      case 5:
        ...
        console.log('PERFORMANCE ENHANCING JELLY')
      ...
      case 6:
        ...
        //WEAPON COMPONENT SHOULD GO HERE
    ...
    if(moveDelay > 0) {
      ...
      if(tasksQueue.length > 0){
        ...
        if(this.state.downPressed) {
          this.tryMoveTo(playerTile.down);
        }
...
//UI
    ...
    return (
      <>
      <GameUI level={level} playerHP={playerHP} xpToNextLevel={xpToNextLevel} playerHPmax={playerHPmax} playerLevel={playerLevel} playerWeapon={playerWeapon} />
      ...
      <div className="field" style={{
        ...
        height:tileSize*(viewportSize*2+1)+'px',
          ...
          {activeParticles.map(particle => {
            return <SpawnParticle html={particle.html} style={particle.style} x={particle.x} y={particle.y}/>
          })}
      </div>
      </>
    )
